...
#define SFE_IPV4_CONNECTION_MATCH_FLAG_QOS_RESOLVED BIT(7)
 ...
 */
static void
sfe_ipv4_update_qos_state(struct sfe_ipv4_connection *c,
			  struct sfe_connection_create *sic)
{
	struct sfe_ipv4_connection_match *cm;
	...
	if (sic->flags & SFE_CREATE_FLAG_REMARK_PRIORITY) {
		...
		cm->flags |= SFE_IPV4_CONNECTION_MATCH_FLAG_PRIORITY_REMARK;
	}
	if (sic->flags & SFE_CREATE_FLAG_REMARK_DSCP) {
		cm->dscp = sic->dscp << SFE_IPV4_DSCP_SHIFT;
		cm->flags |= SFE_IPV4_CONNECTION_MATCH_FLAG_DSCP_REMARK;
	}
	cm->flags |= SFE_IPV4_CONNECTION_MATCH_FLAG_QOS_RESOLVED;
}
...
int sfe_ipv4_create_rule(struct sfe_connection_create *sic)
{
	...
	if (c != NULL) {
		 ...
		 */
		sfe_ipv4_update_protocol_state(c, sic);
		sfe_ipv4_update_qos_state(c, sic);
		spin_unlock_bh(&si->lock);
							   ...
							   int *total_read, struct sfe_ipv4_debug_xml_write_state *ws)
{
	...
	if (!(original_cm->flags &
		...
		if (original_cm->flags &
		    SFE_IPV4_CONNECTION_MATCH_FLAG_PRIORITY_REMARK)
		...
		if (original_cm->flags &
			...
			snprintf(src_dscp, sizeof(src_dscp), "0x%x",
				 original_cm->dscp >> SFE_IPV4_DSCP_SHIFT);
		else
			snprintf(src_dscp, sizeof(src_dscp), "-");
	}
	...
	if (!(reply_cm->flags & SFE_IPV4_CONNECTION_MATCH_FLAG_QOS_RESOLVED)) {
		...
		if (reply_cm->flags &
			...
			snprintf(dest_dscp, sizeof(dest_dscp), "0x%x",
				 reply_cm->dscp >> SFE_IPV4_DSCP_SHIFT);
		else
			snprintf(dest_dscp, sizeof(dest_dscp), "-");
	}
